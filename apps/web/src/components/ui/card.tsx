  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Soft elevation, slightly stronger than ring-1
        "shadow-[var(--shadow-soft)]",
        "has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0",
        "data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0",
        "*:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
        interactive &&
          "cursor-pointer transition-[box-shadow,transform,border-color] duration-200 hover:-translate-y-0.5 hover:border-border hover:shadow-[var(--shadow-elevated)] active:translate-y-0",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center gap-2 rounded-b-xl border-t border-border/60 bg-muted/40 px-5 py-3 group-data-[size=sm]/card:px-4 group-data-[size=sm]/card:py-2.5",
        className
      )}
      {...props}
    />
